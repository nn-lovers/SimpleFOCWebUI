...
class DeviceJoggingControl(QtWidgets.QGroupBox):
    def __init__(self, parent=None):
        ...
        onlyFloat = QtGui.QRegExpValidator(QtCore.QRegExp("[+-]?([0-9]*[.])?[0-9]+"))
    ...
    def joggingStop(self):
        ...
        if (
            controltType == SimpleFOCDevice.ANGLE_CONTROL
            or controltType == SimpleFOCDevice.ANGLE_OPENLOOP_CONTROL
        ):
            self.device.sendTargetValue(self.device.angleNow)
        if (
            controltType == SimpleFOCDevice.VELOCITY_CONTROL
            or controltType == SimpleFOCDevice.VELOCITY_OPENLOOP_CONTROL
        ):
