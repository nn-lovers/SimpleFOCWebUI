...
class SimpleFOCGraphicWidget(QtWidgets.QGroupBox):
    ...
    def __init__(self, parent=None):
        ...
        for sig, sigColor, checkBox, tooltip in zip(
            self.signals,
            self.signalColors,
    ...
    def signalPlotFlagUpdate(self):
        ...
        for i, (checkBox, plotFlag) in enumerate(
    ...
    def upDateGraphic(self, signalList):
        if (
            self.currentStatus is self.connectedPlottingStartedState
            or self.currentStatus is self.connectedPausedState
        ):
...
class ControlPlotPanel(QtWidgets.QWidget):
    def __init__(self, parent=None, controllerPlotWidget=None):
        ...
        spacerItem = QtWidgets.QSpacerItem(
            100, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Maximum
        )
    ...
    def startStoPlotAction(self):
        if (
            ...
            is self.controlledPlot.initialConnectedState
        ):
            # Start pressed
    ...
    def pauseContinuePlotAction(self):
        if (
            self.controlledPlot.currentStatus
            is self.controlledPlot.connectedPausedState
        ):
            # Continue pressed
            self.pauseContinueButton.setText("Pause")
            self.pauseContinueButton.setIcon(GUIToolKit.getIconByName("pause"))
    ...
    def updateMonitorVariables(self):
        self.device.sendMonitorVariables(
            [
                self.signalCheckBox[0].isChecked(),
