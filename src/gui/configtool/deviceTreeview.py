...
class DeviceTreeView(QTreeWidget):
    def __init__(self, parent=None):
        ...
        self.voltageLimit.setFlags(self.voltageLimit.flags() | QtCore.Qt.ItemIsEditable)
        ...
        self.currentLimit.setFlags(self.currentLimit.flags() | QtCore.Qt.ItemIsEditable)
        ...
        self.phaseRes.setFlags(self.phaseRes.flags() | QtCore.Qt.ItemIsEditable)
        ...
        self.selectModulation.addItems(
            ["Sine PWM", "Space Vector PWM", "Trapezoidal 120", "Trapezoidal 150"]
        )
    ...
    def addCustomCommand(sefl, selectedItem):
        ...
        customCommand.setFlags(customCommand.flags() | QtCore.Qt.ItemIsEditable)
    ...
    def addPIDSubtree(self, parent, label):
        ...
        proportionalGain.setFlags(proportionalGain.flags() | QtCore.Qt.ItemIsEditable)
        ...
        derivativeGain.setFlags(derivativeGain.flags() | QtCore.Qt.ItemIsEditable)
    ...
    def refreshDeviceTree(self):
        ...
        self.refreshPIDSubtree(
            self.PIDVelocityConfig, self.device.PIDVelocity, self.device.LPFVelocity
        )
        self.refreshPIDSubtree(
            self.PIDAngleConfig, self.device.PIDAngle, self.device.LPFAngle
        )
        self.refreshPIDSubtree(
            self.PIDCurrentQConfig, self.device.PIDCurrentQ, self.device.LPFCurrentQ
        )
        self.refreshPIDSubtree(
            self.PIDCurrentDConfig, self.device.PIDCurrentD, self.device.LPFCurrentD
        )
